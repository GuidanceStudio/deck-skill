...
assert_grep "$RENDER_SH" 'brave-browser|brave\b' "probes Brave (chromium derivative)"
assert_grep "$RENDER_SH" 'Using: \$BROWSER' "prints which browser was selected"
assert_grep "$RENDER_SH" 'falling back to firefox' "warns when falling back to firefox"
